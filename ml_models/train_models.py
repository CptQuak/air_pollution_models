...
class Args:
    def __init__(self):
        self.STORING_DIR = "trained_models"
        self.DATA_PATH = "../data"
...
def main(args):
    ...
    # rnn parameters
    rnn_params = [
        {'num_hidden': 150, 'enc_num_layers': 2, 'dec_num_layers': 1},
        # {'num_hidden': 150, 'enc_num_layers': 3, 'dec_num_layers': 2}, # UCZYLO ZBYT DLUGO AZ ACTIONS NIE SKONCZYL SIE
    ]
    ## dnn parameters
    dnn_params = [
        {'hidden_list': [128, 256, 128]}, 
        {'hidden_list': [256, 512, 256]}, 
        {'hidden_list': [256, 512, 1024, 512, 256]},
        {'hidden_list': [256, 512, 1024, 2024, 1024, 512, 256]},
    ]
    list_of_models = [
        ('ar_seq2seq', models.ARmodel, seq_gen, ar_params),
        ('ar_seq2seq_pca', models.ARmodel, seq_gen_pca, ar_params),
        ('dnn_seq2seq', models.DNN_model, seq_gen, dnn_params),
        ('rnn_seq2seq', models.RNN_S2S, seq_gen, rnn_params),
        ('baseline', models.BaseLine, seq_gen, ar_params),
    ]
